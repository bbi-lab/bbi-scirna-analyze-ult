...
def make_data_file_json(merge_map, tsv_path):
  merge_jobs = []
  ...
  for (sample_id, process_group, pcr_pair), lanes in sorted(merge_map.items()):
    ...
    merge_jobs.append({
      ...
      'out_file': '%s_%s.hash_reads.merged.tsv' % (sample_full, pcr_pair),
      'in_file_list': [
        '%s/%s-%03d_%s-L%03d.hash_reads.tsv' %
        (tsv_path, sample_id, lane, pcr_pair, lane)
        for lane in sorted(lanes)
      ],
    })

  with open('merge_hash_reads.json', 'w') as fh:
...
#  make_data_file_json(data_file_dict, args.tsv_path)
  ...
  make_data_file_json(merge_map, args.tsv_path)
