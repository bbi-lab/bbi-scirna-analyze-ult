...
if __name__ == '__main__':
  parser = argparse.ArgumentParser(description='A program to hash read files.')
  parser.add_argument('-i', '--input', required=True, default=None, nargs='+', help='Input TSV file paths (required string).')
  parser.add_argument('-o', '--output', required=True, default=None, help='Output TSV filename (required string).')
  ...
  with open(out_file, 'w', newline='') as ofh:
    csv_writer = csv.writer(ofh, delimiter='\t', quoting=csv.QUOTE_NONE, lineterminator='\n')
    header_ref = list()
        ...
        try:
          ...
          if(len(header_ref) != 0):
            ...
            if(header_write_flag == True):
              print('Error: inconsistent condition.', file=sys.stderr)
              sys.exit(1)
            header_ref = header
